...
|-------|-------|-------------------|------------|
| 1 | A |EnhancedTouchSupport.Enable() está en Start. Si el GameObject se desactiva y vuelve a activar, no se ejecutará de nuevo, dejando la app sin touch.| Mover esta línea dentro del método OnEnable().|
| 2 | B |La suscripción al evento (+= AlTocar) está en Start. Por la misma razón anterior, debe estar ligada al ciclo de vida activo del objeto. |Mover la suscripción dentro del método OnEnable(). |
| 3 | C |Utiliza OnDestroy(). Si el GameObject solo se oculta (SetActive(false)), seguirá escuchando toques | Cambiar el nombre del método OnDestroy() a OnDisable().|
| 4 | D | Instancia un nuevo objeto List<ARRaycastHit> usando new en cada toque. Esto satura la memoria (Garbage Collection spikes) provocando tirones/lag.| Declarar private List<ARRaycastHit> hits = new List<ARRaycastHit>(); como variable global de la clase.|
...
|------------|-----------|--------|------|--------------|
|InputMuseoAR |Interacciones |TapPieza |Button | -|
|InputMuseoAR|Interacciones |DobleTapPanel |Button |- |
|InputMuseoAR|Interacciones |SwipePanel |Value |Vector2 |
|InputMuseoAR|Interacciones |PinchZoom |Value |v |
|InputMuseoAR|Interacciones |LongPress |Button |- |
...
public class InputMuseoAR : MonoBehaviour
{
    ...
    public InputActionReference accionTap;
    public InputActionReference accionDobleTap;
    public InputActionReference accionSwipe;
    public InputActionReference accionPinch;
    public InputActionReference accionLongPress;

    void OnEnable()
    {
        accionTap.action.performed += AlHacerTap;
        accionDobleTap.action.performed += AlHacerDobleTap;
        accionSwipe.action.performed += AlHacerSwipe;
        accionPinch.action.performed += AlHacerPinch;
        accionLongPress.action.performed += AlHacerLongPress;

        accionTap.action.Enable();
        accionDobleTap.action.Enable();
        accionSwipe.action.Enable();
        accionPinch.action.Enable();
        accionLongPress.action.Enable();
    }
    ...
    void OnDisable()
    {
        ...
        accionTap.action.Disable();
        accionDobleTap.action.Disable();
        accionSwipe.action.Disable();
        accionPinch.action.Disable();
        accionLongPress.action.Disable();
    }

    private void AlHacerTap(InputAction.CallbackContext ctx) => Debug.Log("Tap ejecutado");
    private void AlHacerDobleTap(InputAction.CallbackContext ctx) => Debug.Log("Doble Tap ejecutado");
    private void AlHacerSwipe(InputAction.CallbackContext ctx) => Debug.Log("Swipe detectado: " + ctx.ReadValue<Vector2>());
    private void AlHacerPinch(InputAction.CallbackContext ctx) => Debug.Log("Pinch detectado");
    private void AlHacerLongPress(InputAction.CallbackContext ctx) => Debug.Log("Guardando en favoritos (Long Press)");
}



**c)** Responde en 3 líneas: ¿qué cambiarías en el código para agregar soporte a Meta Quest 3 en 6 meses?
No cambiaría absolutamente nada en el código. Como el script utiliza InputActionReference, la arquitectura está desacoplada. Solamente abriría el archivo Input Actions Asset en el editor de Unity y le agregaría un nuevo Binding (ej. Right Controller Trigger) a las acciones ya existentes.
---
